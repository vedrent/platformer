...
bool Character::OnFloor(Texture level){
    for (int size = 0; size < width; size++){
        if (GetPixel(SCREEN_WIDTH/2 - width/2 - curr_x + size, curr_y + height, level.GetSurface()) != GetPixel(0, 0, level.GetSurface())){
            return true;
        }
    }
    return false;
...
bool Character::InTexture(Texture level){
    for (int size = 6; size < width-6; size++){
        if (GetPixel(SCREEN_WIDTH/2 - width/2 - curr_x + size, curr_y + height - 1, level.GetSurface()) != GetPixel(0, 0, level.GetSurface())){
            return true;
        }
    }
    return false;
...
int Character::CollisionWithWalls(Texture level){
    ...
    for (int h = 0; h < height; h++){
        if (GetPixel(SCREEN_WIDTH/2 + width/2 - curr_x - 1, curr_y + h, level.GetSurface()) != GetPixel(0, 0, level.GetSurface())){
    ...
    for (int size = 0; size < width; size++){
        if (GetPixel(SCREEN_WIDTH/2 - width/2 - curr_x + size, curr_y + 1, level.GetSurface()) != GetPixel(0, 0, level.GetSurface())){
